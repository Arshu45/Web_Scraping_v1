import sys, os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import streamlit as st
import plotly.graph_objects as go
import plotly.express as px

from utils.styles import apply_css, BRAND_COLORS, PLOT_LAYOUT, kpi_card
from utils.db import get_kpis, get_brand_discount_overview

st.set_page_config(page_title="Overview | Market Intelligence", page_icon="🏠", layout="wide")
apply_css()

st.markdown("<h1 style='color:#fff; font-weight:800;'>🏠 Executive Overview</h1>", unsafe_allow_html=True)
st.markdown("<p style='color:#888;'>High-level KPIs and the overall discount gap across all brands.</p>", unsafe_allow_html=True)
st.markdown("---")

# ── KPI Cards ──────────────────────────────────────────────────────────
with st.spinner("Loading data..."):
    kpis = get_kpis()
    df_overview = get_brand_discount_overview()

c1, c2, c3, c4, c5 = st.columns(5)
kpi_data = [
    (c1, "Our Store Products", f"{kpis['our_products']:,}", "Internal catalog"),
    (c2, "Competitor Products", f"{kpis['comp_products']:,}", "Forever New + Vero Moda"),
    (c3, "Our Avg Discount", f"{kpis['our_avg_disc']}%", "Across all categories"),
    (c4, "Forever New Avg Disc", f"{kpis['fn_avg_disc']}%", f"+{round(kpis['fn_avg_disc'] - kpis['our_avg_disc'], 1)}% vs us"),
    (c5, "Vero Moda Avg Disc", f"{kpis['vm_avg_disc']}%", f"+{round(kpis['vm_avg_disc'] - kpis['our_avg_disc'], 1)}% vs us"),
]
for col, label, value, sub in kpi_data:
    with col:
        st.markdown(kpi_card(label, value, sub), unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)

# ── Grouped Bar: Avg Discount by Category ──────────────────────────────
st.markdown("<div class='section-header'>Average Discount by Category</div>", unsafe_allow_html=True)

brands = ["Our Store", "Forever New", "Vero Moda"]
categories = sorted(df_overview["master_category"].unique())

fig = go.Figure()
for brand in brands:
    bdf = df_overview[df_overview["brand"] == brand]
    cat_vals = [float(bdf[bdf["master_category"] == c]["avg_discount"].values[0]) if c in bdf["master_category"].values else 0 for c in categories]
    fig.add_trace(go.Bar(
        name=brand,
        x=categories,
        y=cat_vals,
        marker_color=BRAND_COLORS.get(brand, "#ccc"),
        opacity=0.9,
        text=[f"{v:.1f}%" for v in cat_vals],
        textposition="outside",
    ))

fig.update_layout(
    **PLOT_LAYOUT,
    barmode="group",
    height=480,
    xaxis=dict(tickangle=-30, gridcolor="rgba(255,255,255,0.05)"),
    yaxis=dict(title="Average Discount (%)", gridcolor="rgba(255,255,255,0.05)", ticksuffix="%"),
    legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
)
st.plotly_chart(fig, width="stretch")

# ── Product Volume Heatmap ─────────────────────────────────────────────
st.markdown("<div class='section-header'>Product Volume by Brand & Category</div>", unsafe_allow_html=True)

pivot = df_overview.pivot(index="brand", columns="master_category", values="products").fillna(0)

fig2 = go.Figure(data=go.Heatmap(
    z=pivot.values,
    x=pivot.columns.tolist(),
    y=pivot.index.tolist(),
    colorscale=[[0, "rgba(108,99,255,0.05)"], [0.5, "rgba(108,99,255,0.4)"], [1, "rgba(108,99,255,0.9)"]],
    text=pivot.values.astype(int),
    texttemplate="%{text}",
    hovertemplate="Brand: %{y}<br>Category: %{x}<br>Products: %{text}<extra></extra>",
))
fig2.update_layout(**PLOT_LAYOUT, height=220, xaxis=dict(tickangle=-30))
st.plotly_chart(fig2, width="stretch")
